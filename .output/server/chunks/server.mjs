import { version, useSSRContext, getCurrentInstance, inject, ref, watchEffect, watch, defineComponent, reactive, onUnmounted, mergeProps, unref, createApp, provide, onErrorCaptured, onServerPrefetch, toRef, h, isReadonly, defineAsyncComponent, isRef } from 'vue';
...
const socket_io_QQJt96c4g9 = /* @__PURE__ */ defineNuxtPlugin((nitro) => {
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    $io.on(SocketEvent.newMessage, (message) => {
      state.allMessages = state.allMessages.concat(message);
    });
    $io.on(SocketEvent.messages, (messages) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "chat-container" }, _attrs))}><div class="chat-header"><h2>Chat Screen</h2></div><div class="chat-messages"><!--[-->`);
      ssrRenderList(unref(state).allMessages, (item, index) => {
        _push(`<div><div class="${ssrRenderClass([item.author === unref(author) ? "sent" : "received", "message"])}"><p>${ssrInterpolate(item.value)}</p></div></div>`);
      });
      _push(`<!--]--></div><form class="chat-input"><input type="text"${ssrRenderAttr("value", unref(state).message)}><button type="submit">Submit</button></form></div>`);
    };
  }
});
...
const _sfc_main = {
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const ErrorComponent = /* @__PURE__ */ defineAsyncComponent(() => import('./error-component-b04ba074.mjs').then((r) => r.default || r));
    const IslandRenderer = /* @__PURE__ */ defineAsyncComponent(() => import('./island-renderer-bd33eec2.mjs').then((r) => r.default || r));
    const nuxtApp = useNuxtApp();
